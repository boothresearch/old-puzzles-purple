 ...
 */
class RParser extends Parser_1.default {
    /**
    ...
    async parse() {
        ...
        if (!name)
            name = path_1.default.basename(this.folder).replace(/[^a-zA-Z0-9]/g, '');
        ...
        pkg.softwareRequirements = await Promise.all(packages.map((name) => this.createPackage(name)));
        return pkg;
    }
    /**
    ...
    async createPackage(name) {
        ...
        pkg.runtimePlatform = 'R';
        if (crandb.URL)
            pkg.codeRepository = crandb.URL.split(','); // See issue #35
        // stencila:SoftwarePackage
        // Create `SoftwarePackage` for each dependency
        if (crandb.Imports) {
            pkg.softwareRequirements = await Promise.all(Object.entries(crandb.Imports).map(([name, version]) => this.createPackage(name)));
        }
        ...
        for (const sysreq of sysreqs) {
            ...
            if (keys.length > 1)
                throw new Error(`Expected on one key for each sysreq but got: ${keys.join(',')}`);
            const name = keys[0];
            ...
            if (typeof debPackage === 'string') {
                ...
                for (const deb of debPackage.filter((deb) => deb.distribution === 'Ubuntu' && deb.releases === undefined)) {
                    if (deb.buildtime) {
                        const required = new schema_1.SoftwarePackage();
